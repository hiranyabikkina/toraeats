package com.tora.toraeats.web.rest;
...
import com.tora.toraeats.config.KafkaProperties;
...
class ToraEatsKafkaResourceIT {
    ...
    void setup() {
        ...
        ToraEatsKafkaResource kafkaResource = new ToraEatsKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/tora-eats-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/tora-eats-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
